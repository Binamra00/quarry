...
# [NEW] Phase 5 Imports (The Analyzer)
from pipeline.analyzer.engine import AnalyzerEngine
from pipeline.analyzer.strategies.causal_impact import CausalImpactStrategy
...
def main():
    ...
    # --- 6. Phase 5: Causal Analysis (The Validator) ---
    # We execute this procedurally AFTER the mining loop.
    # It requires the Ground Truth file generated by Phase 4.
    if args.stage in ["all", "analyze"] and pipeline_healthy:
        print("\n--- 🧠 Phase 5: Causal Analysis (Ground Truth Verification) ---")

        # 1. Setup Paths
        # Use the name from the repo object (e.g. 'commons-lang')
        repo_name = target_repo.name
        input_parquet = config.OUTPUTS_PATH / f"ground_truth_{repo_name}.parquet"
        output_parquet = config.OUTPUTS_PATH / f"final_ground_truth_{repo_name}.parquet"
        matrix_config = config.REPO_ROOT / "pipeline" / "analyzer" / "config" / "relevance_matrix.json"

        # 2. Validation Checks
        if not input_parquet.exists():
            print(f"❌ Error: Ground Truth file not found: {input_parquet.name}")
            print("   👉 Hint: Run '--stage correlate' first.")
            pipeline_healthy = False

        elif not matrix_config.exists():
            print(f"❌ Error: Relevance Matrix not found: {matrix_config}")
            pipeline_healthy = False
            ...
            try:
                ...
                strategies = [CausalImpactStrategy()]
                engine = AnalyzerEngine(strategies, matrix_config)

                # Run Analysis
                engine.run(input_parquet, output_parquet)
